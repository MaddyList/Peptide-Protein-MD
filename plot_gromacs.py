...
def clean_label(label):
    """Converts GROMACS formatting tags into LaTeX-style formatting for Matplotlib, substituting nm with Å."""
    if not label:
        return ""
    
    # Handle GROMACS standard notation conversions
    label = label.replace(r"\s", "^")
    label = label.replace(r"\S", "")
    label = label.replace(r"\N", "")
    label = label.replace(r"\m", "")
    
    # Convert nanometers (nm) to Angstroms (Å)
    label = label.replace("nm", "Å")
    
    # Ensure SASA Å2 gets written as LaTeX Å$^2$
    label = label.replace("Å2", "Å$^2$")
    label = label.replace("Å^2", "Å$^2$")
    
    # General superscript replacement for other GROMACS tags (like ^2, ^3)
...
def plot_xvg(file_name):
    """Generates and saves a clean, standalone plot for a given .xvg file with values scaled to Å."""
    ...
    print(f"Processing: {file_name} -> Converting units and saving to {OUTPUT_DIR}/")

    lower_name = file_name.lower()
    
    # Determine scale factor and handle PCA conversions
    is_pca = "2dproj" in lower_name
    is_sasa = "sasa" in lower_name
    is_hbond = "hb_num" in lower_name or "hbond" in lower_name
    is_rmsf = "rmsf" in lower_name

    if is_pca:
        # Scale both dimensions (PC1 and PC2) by 10 (nm -> Å)
        data[:, 0] = data[:, 0] * 10.0
        data[:, 1] = data[:, 1] * 10.0
    elif is_sasa:
        # Scale area by 100 (nm^2 -> Å^2)
        data[:, 1:] = data[:, 1:] * 100.0
    elif not is_hbond:
        # Scale standard distances by 10 (nm -> Å)
        data[:, 1:] = data[:, 1:] * 10.0
    ...
    # Override / fine-tune Y-axis labels based on metric types
    if is_hbond:
        ax.set_ylabel("Number of Hydrogen Bonds")
    elif is_rmsf:
        ax.set_ylabel("RMSF ($\mathregular{Å}$)")
    elif is_sasa:
        ax.set_ylabel("SASA ($\mathregular{Å^2}$)")
    else:
...
if __name__ == "__main__":
    ...
    print(f"\nSuccess! All 600 DPI plots with customized Y-axis labels and Angstrom conversions are safely saved inside the '{OUTPUT_DIR}' folder.")